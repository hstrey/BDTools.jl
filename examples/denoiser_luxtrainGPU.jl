using HDF5: HDF5
using Random, Lux, LuxCUDA, Zygote, Printf, MLUtils, Statistics, Optimisers
using BSON
...
stand_i_train = standardize(i_train) |> gdev
...
function training(eta, lambda, epochs)
    ...
    for epoch in 1:epochs
        for (i, X) in enumerate(train_dataloader)
            ...
            c = pcor(vec(model_d),vec(stand_o_test))
            if c > bestc
        ...
        @printf "Epoch %d, eta %.4f lambda %.4f Best Corr: %.7f\n" epoch eta lambda bestc
    end
    return bestc, best_ps, best_st
end

lambda_list = pushfirst!([2.0^i for i in -5:4],0) .* 0.001
eta_list = [1/2^i for i in 0:10] .* 0.1

bc_array = zeros(11,11)
result_list = []

for i,l in enumerate(lambda_list)
    for j,e in enumerate(eta_list)
        bc, bp, bs = train(e,l,150)
        bc_array[i,j] = bc
        push!(result_list,(l,e,bc,bp,bs))
    end
end

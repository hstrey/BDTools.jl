...
begin
    # import Pkg
    # Pkg.activate(".")
	# for p in ["Plots", "NIfTI", "Images", "ImageSegmentation", "LsqFit", "Optim", "Interpolations", "DataFrames", "CSV"]
	# 	Pkg.add(p)
	# end
	# Pkg.develop(path="..") # BDTools
	using Revise
	using Plots
	using NIfTI
...
# ╔═╡ 499e52f4-9d52-4c87-90f5-5e15b7e144ab
...
staticEs = let zs = 1:sz[3],
		       res = zeros(9,length(zs))
	for i in zs
		res[:,i] = BDTools.getellipse(view(staticimgs,:,:,i), verbose=false)
	end
	res
end
...
begin
	p = plot(staticEs[1,:], label="x0", legend=:top, ylabel="x0")
	scatter!(p, staticEs[1,:], color=:blue, label=:none)
	q = twinx(p)
	plot!(q, staticEs[2,:], color=:red, label="y0", ylabel="y0")
end

# ╔═╡ 497edfd4-8c69-41cd-a9fb-35a3cf35c23b
md"**Fit line through the slices' centers & determine a deviation angle from `z`-axis**"

# ╔═╡ 52ebeccd-30b8-4586-b293-0782d6d8abed
analysisEs = let skipSlices = [1,5,6,10,13]
	skipIndexes = [i ∉ skipSlices for i in 1:sz[3]]
	staticEs[1:2, skipIndexes]
end

# ╔═╡ 1136012f-cc9f-418c-8475-3fb0df9ea79c
centersFit = BDTools.fitline(analysisEs)
...
θ = let r = π/2-atan(centersFit.slope)
...
let x = staticEs[1,:], y=staticEs[2,:], rng=collect(-1:0.15:1.)
	...
	# show predicted center's line
	cc = centersFit.fn.(rng)
	xp = map(first, cc)
	yp = map(last, cc)
	xy2 = centersFit.fn.(rng)
	xx = centersFit.fnx.(y)
	yy = centersFit.fny.(x)
	plot!(p, xp, yp, label="fit")
	scatter!(p, xp, yp, label="fit centers")
	scatter!(p, x, yy, label="x-fit")
	scatter!(p, xx, y, label="y-fit")
	# show adjusted centers
	# scatter!(p, centers[1,:], centers[2,:], label="z-adj centers")
	# plot!(p, centers[1,:], centers[2,:], label="z-adj")
end
...
cc = let iidxs = (43, 43, 1), θ = staticEs[5],
	...
	rvfn = BDTools.rotatevoxel(origin, last) # create rotation function
...
phantom_itp = BDTools.phantominterp(staticimgs);
...
md"""
Generate image from an average volume slice `Z` by rotating it at an angle `θ` ∈ [-π,π].
...
let θ = deg2rad(theta)
	x_first = staticEs[1,1]   # first slice x-center
	x_last = staticEs[1,end]  # last slice x-center
	a = [x_first, centersFit.fny(x_first), 1]  # line from static average centers
	b = [x_last, centersFit.fny(x_last), 13]
	# generate image
	coords = BDTools.simulated_coordinates_at_z(sz, sliceId, a, b, θ)
...
γ = let yc = staticEs[2, sliceId2],   # slice y-center
...
[deps]
BDTools = "6f2079fd-6c9e-4521-946b-68ee3d3b6add"
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "7464bd6227765c4313f135b1476361dc98fd73b0"
...
deps = ["Artifacts", "JLLWrappers", "Libdl", "Pkg"]
git-tree-sha1 = "9052734dae1fc8015cb9944fdfdf9e569b938761"
